...
class EndlessTMaze:
    """
    Стандартизированная реализация T-Maze, поддерживающая несколько коридоров
    и переменную длину коридоров.
    """
    def __init__(self, length_config: Dict, num_corridors: int = 3, penalty: float = 0.0,
        ...
        # Динамически устанавливаемая длина коридора
        ...
        if hints_override is not None:
            assert len(hints_override) == num_corridors, "hints_override length must match num_corridors"
    ...
    def reset(self):
        # При каждом сбросе генерируем новую длину коридора, если режим "uniform"
        ...
        if self.x < self.corridor_length:
            ...
            if action == correct_turn_action:
                ...
                if self.current_corridor < self.num_corridors:
                    # Сбрасываем позицию для нового коридора
                    self.x = 0
                    self.current_hint = self.hints[self.current_corridor]
                    # Генерируем новую длину для следующего коридора, если нужно
                    if self.length_mode == "uniform":
                         self.corridor_length = random.randint(self.corridor_length_min, self.corridor_length_max)
                else:
                    self.done = True
